...
pub fn sync_shadow_dom(cursor: &mut Option<usize>, offset: &Cell<usize>, have: &Node, want: ShadowDom) -> Node {
    fn recurse(cursor: &mut Option<usize>, offset: &Cell<usize>, have: &Node, want: ShadowDom) -> Node {
        ...
        match want {
            ...
            ShadowDom::Element(want) => {
                ...
                while i < have_children_len {
                    ...
                    recurse(cursor, offset, &have_child, want_child);
                    i += 1;
                }
                ...
                for want_child in want_children_iter {
                    ...
                    recurse(cursor, offset, &child, want_child);
                }
                return have.dyn_into().unwrap();
            },
        }
    ...
    let have = recurse(cursor, offset, have, want);
    if cursor.is_some() {
        dom::select(&have, 0);
    }
    return have;
